...
- **ロケーション設定**（`app.locations.jsx`）  
  **並び順**（セレクト）、**エリア設定**（エリアでグルーピング・エリアごとに折りたたみ・未設定の見出し・グループ一覧と並び順・エリアを追加）、**近隣店舗表示設定**（近隣店舗を表示チェック、その他ロケーションの見出し入力）、**店舗受取設定**（「この店舗で受け取る」ボタン表示・ボタンラベル・チェックアウトへリダイレクトするチェック）、**ロケーション名のリンク設定**（「リンクを表示する」一括チェック。ON のとき一覧で URL を入力したロケーションのみリンク表示）、**ロケーション一覧**テーブル（ロケーション名・公開名・エリア・並び順・上部固定・表示・近隣除外・**リンクURL**）。配送対応／ローカルデリバリー対応／店舗受け取り対応は API から行ごとに表示。テーブルは幅が足りない場合に横スクロール可能（`minWidth` と `overflow-x: auto`）。エリア見出し・ボタンの**デザイン**は管理画面にはなく、テーマのカスタマイザー（ブロックスキーマ）で設定する。
...
- **整理方針**: カスタマイザーで見やすいよう、**タイトルとレイアウト**／**凡例・注意書き・行・枠線・色**／**エリア見出しのデザイン**／**店舗受け取りボタンのデザイン** のグループに分けてある（`header` / `paragraph` で区切り）。
...
| 凡例・注意書き・行・枠線・色 | 凡例を表示、凡例の位置・揃え・色、注意書きを表示（文言はアプリ）、注意書きの位置・揃え・間隔・色、リスト区切り文字、行内の表示内容、行の上下余白・左右余白、ロケーション列の幅、枠線スタイル・太さ・色、ステータス色、在庫あり/残りわずか/在庫なしマークの色、エリア折りたたみの開閉マーク |
| エリア見出しのデザイン | エリア見出しの背景色・文字色・文字サイズ・太さ・余白 |
| 店舗受け取りボタンのデザイン | ボタン背景色・文字色・角丸・余白・文字サイズ |
| その他 | 追加CSS |
...
| **ロケーション設定** | 並び順、上部固定ロケーション、エリア設定（グルーピングON/OFF、折りたたみON/OFF、未設定の見出し、グループ一覧）、近隣店舗を表示、その他ロケーションの見出し、店舗受け取りボタン表示・ボタンラベル・チェックアウトへリダイレクト、**ロケーション名のリンク設定**（リンクを表示する一括チェック、行ごとリンクURL）、ロケーション一覧（公開名・エリア・並び順・上部固定・表示・近隣除外・**リンクURL**） |
