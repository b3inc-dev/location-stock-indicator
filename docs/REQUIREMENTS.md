...
| **管理画面（在庫表示設定）** | 在庫の「意味」・文言・ルールの一元管理 | 全商品で共通。「在庫なし」の境界値、マークの文字、ラベル・メッセージ・注意書きの文言、クリック時の挙動など。 |
| **管理画面（ロケーション設定）** | どのロケーションをどう並べるか・機能のON/OFF | データに近い。並び順、エリアグループ、近隣店舗の表示、店舗受け取りのON/OFF・ボタン文言・リダイレクト、ロケーション一覧の公開名・表示・近隣除外など。 |
...
| 画面 | 項目の要約 |
|------|------------|
| **在庫表示設定** | 閾値（在庫なし/在庫ありの境界）、在庫マーク（◯△✕の文字）、在庫数ラベル・前後の文字、ステータスラベル（在庫あり/残りわずか/在庫なしの文言）、メッセージ文言（読み込み中・在庫なし・エラー）、注意書きの文言、クリック設定（なし/地図/URL） |
| **ロケーション設定** | 並び順、上部固定ロケーション、エリア設定（グルーピングON/OFF、折りたたみON/OFF、未設定の見出し、グループ一覧）、近隣店舗を表示、その他ロケーションの見出し、店舗受け取りボタン表示・ボタンラベル・チェックアウトへリダイレクト、ロケーション一覧（公開名・エリア・並び順・上部固定・表示・近隣除外） |
...
| 閾値・在庫マークの文字・ラベル・クリック設定 | **管理画面（在庫表示設定）** | ルール・文言 |
| マークの「色」 | **スキーマ** | テーマごとの色 |
| 行の表示内容・区切り・枠線・エリア折りたたみマーク | **スキーマ** | 表示形式・見た目 |
| 並び順・エリア・近隣・店舗受け取りのON/OFF・ボタン文言・その他見出し | **管理画面（ロケーション設定）** | データ・機能 |
