...
- **表示ルール**: 「この店舗で受け取る」ボタンは、**受け取り可能設定が有効な店舗（ロケーション）の行にのみ**表示されます。Shopify のロケーションで「店舗受け取り」が有効（`localPickupSettingsV2` が設定されている）かどうかは App Proxy が取得し、各在庫行に `storePickupEnabled` として渡しています。受け取り不可の店舗の行ではボタンは表示されません。
...
- **リダイレクト OFF 時**:
  - タップ直後はボタンが**読み込み中**（ぐるぐるスピナー＋「追加中...」など管理画面で設定した文言）になり、追加成功後に**成功メッセージ**（既定は「追加しました」）を表示し、約 2 秒で元のラベルに戻る。
  ...
  - スニペット側で **Section Rendering API** により、`cart-drawer` や `cart-icon-bubble` などよくあるセクション ID の DOM 差し替えを試みるため、該当するテーマではリロードなしでカート表示が更新される場合がある。
